...
class NewsCurator:
    ...
    def curate(
        ...
        """Curate a list of headlines into grouped, scored, translated results.

        Returns CuratedGroup list covering ALL input headlines.
        """
        if not titles:
            return []
    ...
    def _fallback(self, titles: list[str]) -> list[CuratedGroup]:
        """LLM 실패 시 개별 헤드라인을 각각 하나의 그룹으로 반환 (fail-open)."""
        return [
            CuratedGroup(
                group_id=str(uuid.uuid4())[:8],
                headline_indices=[i],
